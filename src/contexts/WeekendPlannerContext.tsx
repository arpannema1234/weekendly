...
interface WeekendPlannerContextType {
...
const WeekendPlannerContext = createContext<
    WeekendPlannerContextType | undefined
>(undefined);

interface WeekendPlannerProviderProps {
    children: ReactNode;
}

export const WeekendPlannerProvider: React.FC<WeekendPlannerProviderProps> = ({
    children,
}) => {
    ...
    const [currentPlan, setCurrentPlan] = useState<WeekendPlan>(() => {
        ...
        const saved = localStorage.getItem("weekendly-current-plan");
        if (saved) {
            try {
    ...
    const value: WeekendPlannerContextType = {
        currentPlan,
        savedPlans,
        addActivityToSchedule,
    ...
    return (
        <WeekendPlannerContext.Provider value={value}>
            {children}
        </WeekendPlannerContext.Provider>
    );
};

export const useWeekendPlannerContext = (): WeekendPlannerContextType => {
    const context = useContext(WeekendPlannerContext);
    if (context === undefined) {
        throw new Error(
            "useWeekendPlannerContext must be used within a WeekendPlannerProvider"
        );
    }
    return context;
};
